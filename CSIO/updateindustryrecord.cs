...
namespace CSIO
{
    public partial class updateindustryrecord : Form
    {
        public updateindustryrecord()
        {
            InitializeComponent();
        }
        ...
        private void updateindustryrecord_Load(object sender, EventArgs e)
        {
            DisplayData();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (result1 == DialogResult.Yes)
            {
                ...
                MySqlCommand cmds = new MySqlCommand("INSERT INTO update_industry_hist(updatid, industryid, industryreg, decisiondate, dartanum, updattitle, updatnew, oldrec, transid, updatedate, updatenepdate, updateuser, tax, comment, updatedreason, updateduser) SELECT updatid, industryid, industryreg, decisiondate, dartanum, updattitle, updatnew, oldrec, transid, updatedate, updatenepdate, updateuser, tax, comment,@updatereason,@updateuser FROM update_industry WHERE updatid= @uid", sqlcon.con);
                ...
                if (p > 0)
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("UPDATE  update_industry SET oldrec= @oldrec,updatnew= @updatrec WHERE updatid= @uid", sqlcon.con);
                    ...
                    if (n > 0)
                    {
                        ...
                        industrysomsodhansearch existing = Application.OpenForms.OfType<industrysomsodhansearch>().FirstOrDefault();
                        if (existing != null)
                        {
                            existing.Show();
